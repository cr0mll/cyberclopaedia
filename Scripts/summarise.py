...
NotesDirectoryList = ["Reconnaissance", "Exploitation", "Post Exploitation", "Malware Analysis", "Malware Development", "Reverse Engineering", "Hardware Hacking" , "Cryptography", "Networking", "Web"]

def SummariseDirectoryRecursively(Dir: str, OutputFile, Counter = 1):
    OutputFile.write("\t" * (Counter - 1) + "- [{}]({})\n".format(Dir[Dir.rfind("/") + 1:], NotesPath.joinpath(Dir).joinpath("README.md").relative_to(NotesPath).as_posix().replace(" ", "%20")))
    ...
    for File in Files:
        if File.name == "README.md" or not File.name.endswith(".md"):
            continue
...
OutputFile.write("[Cyberclopaedia](README.md)\n")
